...
(Sec.~\ref{sec:trainingdata}). The users involved in the pilot study are different from those participated in the experiments for
 evaluating the developed system (Sec.~\ref{sec:evalusers}).
...
\cparagraph{Hands on the head.}  Fig.~\ref{Bodyhand} shows change of rotation angle using the gyroscope for one of our pilot study users
...
In general, there are six possible body rolover transitions. These include four posture transitions between the supine and laterals (left
...
\cparagraph{Noise cancelling.} One of the challenges in detecting micro-body movements is to cancel the inherent noises brought by the
...
\cparagraph{Acoustic feature calculation.} In order to identify different acoustic events accurately, we select the short-term average
...
\cparagraph{Determining the start and the end points.} Traditional end-point detection algorithms~\cite{stowell2015detection} are
...
they typically do not have a proximity sensor.  The key to deal with this practical challenge is that the illumination would drop suddenly
