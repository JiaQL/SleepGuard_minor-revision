...
\cparagraph{The accuracy of sleep stage detection.} {In our experiment, we were unable to directly compare {\systemname} against medical
...
\cparagraph{Battery life.} A critical design requirement for sleep detection is that the monitoring can operate sufficiently long to cover
...
\cparagraph{Sensor data.} One limitation of {\systemname} is that we have not taken advantage of heart rate when determining the current
...
\cparagraph{Single wrist sensor.} In our paper, we only use the sensor data of only one wrist, which loses efficiency for detecting
...
\cparagraph{Comparing to Fitbit.} In addition, when we use Fitbit as groundtruth, Fitbit is worn on a different wrist from {\systemname}.
...
\cparagraph{Multiple-sleeper scenario.} Currently, {\systemname} considers that the user is sleeping alone, but there are still more
...
\cparagraph{Occurrence of unusual the arm's positions.} We detect sleep posture based on arm's positions and focus on three specific positions
